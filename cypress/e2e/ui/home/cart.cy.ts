...
describe('Cart functionality', () => {
  ...
  it('should correctly handle adding, removing and persisting multiple cart products', () => {
    ...
    const productsToAdd = [products[0].name, products[1].name, products[2].name, products[3].name]

    // List of products that will be removed from the cart later in the test
    const productsToRemove = [products[2].name, products[3].name]
    ...
    productsToAdd.forEach((name, i) => {
      ...
      homePage.elements.shoppingCartBadge().should('have.text', String(i + 1))
    })
    ...
    productsToRemove.forEach((name, i) => {
      ...
      cartPage.elements.cartProducts().should('have.length', productsToAdd.length - (i + 1))
      homePage.elements.shoppingCartBadge().should('have.text', productsToAdd.length - (i + 1))
    })
    ...
    homePage.elements.shoppingCartButton().click()
    cartPage.elements.cartProducts().should('have.length', remainingProducts.length)
    ...
    homePage.elements.shoppingCartButton().click()
    cartPage.elements.cartProducts().should('have.length', 0)
