...
describe('Order finalization', () => {
  ...
  it('should add products to cart, fill in shipping details, and verify order totals including tax', () => {
    ...
    const productsToAdd = [products[0].name, products[1].name, products[4].name, products[5].name]
    ...
    homePage.elements.shoppingCartButton().click()
    cartPage.elements.cartProducts().should('have.length', productsToAdd.length)
    ...
    checkoutPage.elements.continueButton().should('be.visible').click()
    overviewPage.elements.title().should('be.visible').should('have.text', 'Checkout: Overview')

    // Verify that all products are displayed on the overview page with correct details
    overviewPage.elements.cartProducts().should('have.length', productsToAdd.length)
    ...
    overviewPage.elements.finishButton().should('be.visible').click()
    finishPage.elements.title().should('be.visible').should('have.text', 'Checkout: Complete!')
    finishPage.elements
      .completeHeader()
      .should('be.visible')
    ...
    finishPage.elements.backHomeButton().should('be.visible').click()
    homePage.elements.products().should('have.length', products.length)
    products.forEach((p) => {
