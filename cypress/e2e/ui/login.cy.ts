...
describe('Successful login and logout', () => {
  ...
  it('should log in with valid credentials and logout', () => {
    homePage.elements.sideBurgerMenuButton().click()
    sideMenuPage.elements.logout().click()
...
describe('Negative scenarios', () => {
  ...
  it('should display an error when logging in with invalid credentials', () => {
    ...
    loginPage.elements.usernameInput().type('invalidUser')
    loginPage.elements.usernameInput().should('have.value', 'invalidUser')

    // Input invalid password
    loginPage.elements.passwordInput().type('passwd')
    loginPage.elements.passwordInput().should('have.value', 'passwd')

    // Attempt to log in and verify error message
    loginPage.elements.loginButton().should('be.visible').click()
    loginPage.elements.errorMessage().should('contain.text', AUTH_ERRORS.INVALID_CREDENTIALS)
  })

  it('should display a empty credentials error when fields are empty', () => {
    // Ensure username is empty
    loginPage.elements.usernameInput().should('be.visible')
    loginPage.elements.usernameInput().should('be.empty')
    ...
    // Attempt to log in and verify error message
    loginPage.elements.loginButton().should('be.visible').click()
    loginPage.elements.errorMessage().should('contain.text', AUTH_ERRORS.EMPTY_CREDENTIALS)
  })

  it('should prevent login when SQL injection payload is submitted', () => {
    // Input SQL injection payload into username field
    loginPage.elements.usernameInput().should('be.visible')
    loginPage.elements.usernameInput().type("admin' OR '1'='1").blur()

    // Input SQL injection payload into password field
    loginPage.elements.passwordInput().should('be.visible')
    loginPage.elements.passwordInput().type("admin' OR '1'='1").blur()
  ...
  it('should prevent login when XSS payload is submitted', () => {
    ...
    loginPage.elements.usernameInput().should('be.visible')
    loginPage.elements.usernameInput().type("<script>alert('XSS')</script>").blur()

    // Input XSS payload into password field
    loginPage.elements.passwordInput().should('be.visible').click()
    loginPage.elements.passwordInput().type('passwd').blur()
